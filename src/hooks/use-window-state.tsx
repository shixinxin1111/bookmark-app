...
import { initialWindowState } from "@/constants/window";
...
export function useWindowState() {
  const [windowState, setWindowState] =
    useState<BookmarkWindowState>(initialWindowState);
  ...
  useEffect(() => {
    ...
    const unsubscribe = bookmarkWindow.onModeChange((state) => {
      if (isMounted) {
        setWindowState(state);
      }
    });
  ...
  async function changeWindowMode(mode: BookmarkWindowMode) {
    const bookmarkWindow = getBookmarkWindowApi();
    ...
    try {
      const nextState = await bookmarkWindow.setMode(mode);
